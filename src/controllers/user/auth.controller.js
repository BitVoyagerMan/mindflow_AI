...
module.exports.login = async (req, res, next) => {
  try {
    ...
    REFRESH_TOKEN.cookie.options.expires = new Date(
...
module.exports.forgotPassword = async (req, res, next) => {
  const MSG = `If ${req.body?.email} is found, we’ll email you instructions to reset your password. Please wait up to 5 minutes and check your spam folder if you don’t see it`;
  try {
    const errors = validationResult(req);
    if (!errors.isEmpty()) {
